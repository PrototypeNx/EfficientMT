...
from ..models.manual_temp_attn import ReferenceTemporalAttentionControl
from ..utils.util import video_preprocess
import pdb
...
@dataclass
class MotionPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]


class MotionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        ref_text_embeddings: torch.FloatTensor,
        ref_timestep: int,
        ref_feat_dict: dict,
        ...
        **kwargs,
        ...
        # step_t = int(dataset_config.add_noise_step)
        ref_step_t = ref_timestep.repeat(batch_size).to(ref_latents.device).long()

        # Prepare extra step kwargs.
        extra_step_kwargs = self.prepare_extra_step_kwargs(generator, eta)

        self.reference_control_reader.clear()
        if self.reference_control_writer is not None:
            self.reference_control_writer.clear()
        # Denoising loop
        num_warmup_steps = len(timesteps) - num_inference_steps * self.scheduler.order
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ############# forward reference motion ##############
                if i == 0:
                    if self.ref_unet is not None:
                        self.ref_unet(
                            ...
                            # torch.cat([torch.zeros_like(ref_latents), ref_latents], dim=0),
                            ref_step_t,
                            encoder_hidden_states=ref_text_embeddings,
                            return_dict=False,
                        )
                        
                        # Update reference unet feature into denosing net
                        self.reference_control_reader.update(self.reference_control_writer)
                    else:
                        self.reference_control_reader.update_ready_made(ref_feat_dict)
        ...
        return MotionPipelineOutput(videos=video)
